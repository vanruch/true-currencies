...
contract('--Full upgrade process with multisig--', function (accounts) {
    ...
    describe('--Set up contracts--', function () {
        beforeEach(async function () {
            ...
            this.fastPauseMints = await FastPauseMints.new({ from: owner1 })
            this.dateTime = await DateTimeMock.new({ from: owner1 })
            this.controllerImplementation = await TimeLockedController.new({ from: owner1 })
            ...
            await this.fastPauseMints.setController(this.controller.address, { from: owner1 })
            await this.fastPauseMints.modifyPauseKey(pauseKey, true, { from: owner1 })
            ...
            await this.registry.setAttribute(this.fastPauseMints.address, "isTUSDMintChecker", 1, "notes", { from: owner1 })
            await this.multisigOwner.setMintLimit(30*10**18, { from: owner1 })
            await this.multisigOwner.setMintLimit(30*10**18, { from: owner2 })
            await this.multisigOwner.setSmallMintThreshold(11*10**18, { from: owner1 })
            await this.multisigOwner.setSmallMintThreshold(11*10**18, { from: owner2 })
            await this.multisigOwner.setMinimalApprovals(2,3, { from: owner1 })
            await this.multisigOwner.setMinimalApprovals(2,3, { from: owner2 })
        ...
        describe('Upgrade each piece of the contract', async function()  {
            it('upgrades token implementation contract', async function() {
                ...
                await this.multisigOwner.requestMint(oneHundred, 10*10**18,  {from: owner1})
                await this.multisigOwner.requestMint(oneHundred, 10*10**18, {from: owner2})
                await this.multisigOwner.finalizeMint(1, {from: owner1})
                await this.multisigOwner.finalizeMint(1, {from: owner2})    
            })
            it('upgrades controller implementation contract', async function() {
                this.newControllerImplementation = await TimeLockedController.new({ from: owner2 })
            ...
            it('upgrades multisig owner contract', async function() {
                ...
                await this.newMultisigOwner.msSetTimeLockController(this.controller.address, {from : owner1 })
                await this.newMultisigOwner.msSetTimeLockController(this.controller.address, {from : owner2 })
            ...
            it('upgrades controller proxy contract', async function() {
                this.newControllerImplementation = await TimeLockedController.new({ from: owner1 })
                this.newControllerProxy = await Proxy.new({ from: owner1 })
                this.newController = await TimeLockedController.at(this.newControllerProxy.address)
